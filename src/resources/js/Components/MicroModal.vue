...
const closeModal = (event) => { if (event.target.classList.contains('modal__overlay')) toggleStatus() }
...
<template>
  <div v-if="isShow" class="modal" id="modal-1" aria-hidden="true">
    <div @click="closeModal" class="modal__overlay" tabindex="-1" data-micromodal-close>
        ...
        <main class="modal__content" id="modal-1-content">
          <div class="w-full mx-auto overflow-auto">
            <table class="table-auto w-full text-left whitespace-no-wrap">
              ...
              <tbody>
                <tr v-for="customer in customers.value.data" :key="customer.id" @click="setCustomer({id: customer.id, kana: customer.kana})"
  ...
  <input class="ml-8 leading-7 text-sm text-gray-600" type="text" v-model="search">
  <button @click="searchCustomer" type="button" data-micromodal-trigger="modal-1" class=" ml-8 text-white bg-teal-500 border-0 py-2 px-8 focus:outline-none hover:bg-teal-600 rounded text-lg">検索する</button>
</template>
